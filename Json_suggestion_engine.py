...
def interactive_field_suggester():
    ...
    while True:
        ...
        for i, field in enumerate(existing_fields, 1):
            print(f"{i}. {json.dumps(field, indent=2)}")

        suggestions = suggest_fields(existing_fields)

        print("\nSuggested fields:")
        numbered_suggestions = {f"{i}": field for i, field in enumerate(suggestions.values(), 1)}
        print(json.dumps(numbered_suggestions, indent=2))
        ...
        try:
            chosen_indices = [int(idx.strip()) for idx in choice.split(',')]
            valid_indices = [idx for idx in chosen_indices if 1 <= idx <= len(suggestions)]

            if valid_indices:
                for idx in valid_indices:
                    selected_field = list(suggestions.values())[idx - 1]
